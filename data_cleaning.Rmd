...
test_data <- read_delim(here("data", "ab13_snow_v1.csv"), skip = 14, delim = ",", na = c("-99", "NaN", "")) %>% 
  ...
  mutate(total_snow_wy = cumsum(daily_snow_accum)) %>% 
  ungroup()
```
```{r}
ggplot(data = test_data, aes(x = month_name_day, y = total_snow_wy)) +
  geom_point(aes(color = water_year)) +
  scale_x_discrete(breaks = c("Oct 01", "Nov 01", "Dec 01", "Jan 01", "Feb 01", "Mar 01", "Apr 01", "May 01", "Jun 01", "Jul 01")) +
